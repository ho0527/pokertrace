...
def multidaytodayentries(sessionid):
	# 本日進場人數 = 此場報名確認(confirmed / advanced)的相異使用者數。
	# 顯示端分母用這個, 不用 multidayremaining(只算前一日晉級進來的人)——
	# 否則 Day2 若有 re-entry / 直接報名, 在場人數會超過晉級人數而出現 13/12 的矛盾畫面。
	# 這裡數「人」(DISTINCT userid) 不是「入場次數」, 一人 re-entry 仍算 1 人, 才能與在場人數(也是數人)對齊。
	# status 含 advanced: 已晉級去下一日的人今天仍是「本日進場」過, 計入分母; 在場人數(numerator)才排除 advanced。
	row=query(SETTING["dbname"],f"""SELECT COUNT(DISTINCT "userid") AS count FROM "sessionplayer" WHERE "sessionid"=%s AND "status" IN ('confirmed','advanced') AND "deletetime" IS NULL""",[sessionid],SETTING["dbsetting"])
	if row and row[0].get("count") is not None:
		return intval(row[0].get("count"),0)
	return 0
...
def multidaysourcetotalentries(sessionid):
	# 整個多日賽的參賽總入場次數 = 走遍同一多日賽的所有場次, 每場加總「本場入場次數 − 晉級進本場的人數」。
	#   本場入場次數 = _sessiontotalentries = SUM(1+rebuy+reentry)（confirmed/advanced）。
	#   減掉「晉級進本場的人數」(_multidayremainingcount) 是為了扣掉重複: 晉級帶過來的人在源頭日
	#   已經算過一次基本入場, 不能在中間日再算一次; 但他們在本日的 rebuy/reentry, 以及本日「非晉級」的
	#   全新報名/re-entry, 都是新的入場, 會被保留下來。
	#   例: Day1=127, Day2 有 12 人晉級 + 1 人 Day2 才進來 → _sessiontotalentries(Day2)=13,
	#       Day2 貢獻 13−12=1, 總計 127+1=128。
	...
	for dayid in visited:
		total=total+_sessiontotalentries(dayid)-_multidayremainingcount(dayid)
	return total
