...
def synctimerplayers(sessionrow):
	...
	for item in registrations:
		row=query(SETTING["dbname"],f"""SELECT*FROM "sessiontimerplayer" WHERE "sessionid"=%s AND "sessionplayerid"=%s""",[sessionrow["id"],item["id"]],SETTING["dbsetting"])
		if row:
			query(SETTING["dbname"],f"""UPDATE "sessiontimerplayer" SET "deletetime"=NULL,"updatetime"=NOW() WHERE "id"=%s""",[row[0]["id"]],SETTING["dbsetting"])
		else:
			queryinsert(SETTING["dbname"],"sessiontimerplayer",{
...
def gettimerplayerrows(sessionid):
	row=query(SETTING["dbname"],
		f"""SELECT tp.*, sp."status" AS registrationstatus, sp."tableid", sp."seatno",
		          ...
		          t."name" AS tablename, t."token" AS tabletoken
		   ...
		   WHERE tp."sessionid"=%s AND tp."deletetime" IS NULL AND sp."status"='confirmed' AND sp."deletetime" IS NULL
		   ORDER BY tp."status" ASC, t."name" ASC, sp."seatno" ASC, tp."id" ASC""",
		[sessionid],
		SETTING["dbsetting"]
	)
	return row or []
...
def readruntime(sessionrow,players,totalentries,schedule):
...
def buildtimerstate(sessionid):
	...
	runtime=readruntime(sessionrow,players,totalentries,schedule)
