...
def createtimertables():
	...
	# 大螢幕品牌設定「不」放在 sessiontimerconfig。
	# 既有實作已經把它們放在 session 表(brandname/brandcolor/brandlogo/displayfields,
	# 見 session.py 的 ensuresessionsettingcolumns), editsession 也已經收這幾個欄位。
	# 本次只補既有實作缺的 columnorder, 加在同一個地方以免品牌設定散在兩張表。
	query(SETTING["dbname"],"""
...
def gettimerreaduser(request):
	...
	# 縱深防禦：與 authhelper.gettokenuser 同一個理由 —— 封禁時已撤銷 token，這裡再擋一次。
	# 這支是**唯讀**用的 helper，而且允許匿名（沒帶 token 時回 (None,None)），
	# 所以被封禁的使用者退化成匿名，而不是回錯誤 —— 匿名看得到的東西他還是看得到，
	# 匿名看不到的（私人場、個人紀錄場）就擋掉了。回錯誤反而會讓公開顯示頁整個壞掉。
...
# 大螢幕可獨立開關的資訊區塊代號。只接受這份清單裡的值, 擋掉前端亂送。
DISPLAYBLOCKLIST=["payout","stack","nextblind","marquee"]
# 三欄的欄位代號, columnorder 是這三個的排列
DISPLAYCOLUMNLIST=["payout","center","info"]

def normalizeaccentcolor(value):
	# 只收 #rgb / #rrggbb。收不到合法值就回空字串, 讓 display 用內建預設色。
...
def normalizebrandlogourl(value):
	# 只收 http / https。擋掉 javascript: 與 data: 之類的協定注入(見 feature-spec FR-10)。
	text=str(value or "").strip()
	if not text:
		return ""
...
def normalizehiddenblock(value):
	# 收字串或陣列, 一律正規化成逗號字串; 只留白名單內的代號並保持固定順序。
	# 空字串代表「沒有設定」= 全部顯示, 與改版前行為一致。
	if isinstance(value,list):
		itemlist=value
	else:
		itemlist=str(value or "").split(",")
	picked=[]
	for item in itemlist:
		name=str(item or "").strip().lower()
		if name in DISPLAYBLOCKLIST and name not in picked:
			picked.append(name)
	output=[]
	for name in DISPLAYBLOCKLIST:
		if name in picked:
			output.append(name)
	return ",".join(output)

def normalizecolumnorder(value):
	# 必須是三個欄位代號的完整排列, 少一個或多一個都視為沒設定, 避免版面缺一欄。
	if isinstance(value,list):
		itemlist=value
	else:
		itemlist=str(value or "").split(",")
	...
	for item in itemlist:
		name=str(item or "").strip().lower()
		if name in DISPLAYCOLUMNLIST and name not in picked:
			picked.append(name)
	if len(picked)!=len(DISPLAYCOLUMNLIST):
		return ""
	return ",".join(picked)

def userdisplaydefault(userid):
	# 場次的品牌設定初值來自「場次擁有者」的個人預設(feature-spec FR-7)。
	# 欄位名對齊 session 表既有的 brandname/brandcolor/brandlogo/displayfields, 只加 display 前綴,
	# 避免同一個概念在兩處用不同名字。讀不到就整組空的 = 沒有品牌, display 維持原本的樣子。
	row=query(SETTING["dbname"],f"""SELECT "displaybrandname","displaybrandcolor","displaybrandlogo","displaydisplayfields","displaycolumnorder" FROM "user" WHERE "id"=%s AND "deletetime" IS NULL""",[userid],SETTING["dbsetting"])
	if not row:
		return {"brandname": "","brandcolor": "","brandlogo": "","displayfields": "","columnorder": ""}
	row=row[0]
	return {
		"brandname": str(row.get("displaybrandname") or "")[:120],
		"brandcolor": normalizeaccentcolor(row.get("displaybrandcolor")),
		"brandlogo": normalizebrandlogourl(row.get("displaybrandlogo")),
		"displayfields": normalizehiddenblock(row.get("displaydisplayfields")),
		"columnorder": normalizecolumnorder(row.get("displaycolumnorder"))
	}
...
def buildtimerstate(sessionid,readonly=False):
	...
	# 送出前一律再正規化一次(TASK-022)：這幾個欄位在 editsession 只被驗成 "string",
	# 舊資料或直接打 API 塞進來的 javascript: / data: 協定, 不能讓它們流到大螢幕上算繪。
	state["brandName"]=str(sessionrow.get("brandname") or "")[:120]
	state["brandColor"]=normalizeaccentcolor(sessionrow.get("brandcolor"))
	state["brandLogo"]=normalizebrandlogourl(sessionrow.get("brandlogo"))
	state["displayFields"]=normalizehiddenblock(sessionrow.get("displayfields"))
	state["columnOrder"]=normalizecolumnorder(sessionrow.get("columnorder"))
	if boolval(sessionrow.get("linkuser")):
...
def savesplitstate(sessionrow,stateval,stateversion=None):
	...
	configdata={
		...
		# 品牌設定不寫在這裡: 它們屬於 session 表, 由 editsession 負責寫入。
		# 一個概念只留一條寫入路徑, 避免兩邊各寫一半互相覆蓋。
		"updatetime": nowtime(),
		"deletetime": None
	}
	if configrow:
