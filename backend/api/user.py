	...
	def signin(request):
		try:
			...
			if not row:
				while True:
					# 產生 8 碼隨機數字
					playerid=str(random.randint(0,99999999)).zfill(8)

					# 判斷是否「沒有」在裡面
					if not query(SETTING["dbname"],f"""SELECT*FROM "user" WHERE "id"=%s""",[playerid],SETTING["dbsetting"]):
						break

				query(SETTING["dbname"],f"""INSERT INTO "user"("token","uid","name","email","avatarurl","permission","verifytoken","createtime","playerid")VALUES(gen_random_uuid(),%s,%s,%s,%s,%s,%s,%s,%s)""",[uid,name,email,picture,"1","0",nowtime(),playerid],SETTING["dbsetting"])
				row=query(SETTING["dbname"],f"""SELECT*FROM "user" WHERE "uid"=%s AND "deletetime" IS NULL""",[uid],SETTING["dbsetting"])
