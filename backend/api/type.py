			...
			if tokenrow:
				...
				if tokenuserrow:
					...
					if requestdata["error"] is None:
						...
						row=query(SETTING["dbname"],f"""SELECT*FROM "type" WHERE "gametype"=%s""",[gametype],SETTING["dbsetting"])

						query(SETTING["dbname"],f"""INSERT INTO "type"("token","userid","gametype","name","clubid","buyin","rebuycount","rebuybuyin","winprice","chip","winthing","starttime","endtime","description","place","totalbuyin")VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)""",[f"{gametype[:2].upper()}{nowtime().split(" ")[0].split("-")[0]}{(str(len(row)+1)).zfill(4)}",tokenuserrow["id"],gametype,name,clubid,buyin,rebuycount,rebuybuyin,winprice,chip,winthing,starttime,endtime,description,place,totalbuyin],SETTING["dbsetting"])
