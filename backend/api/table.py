...
from .hand import sessionaccess,boolval,intval
			...
			if tokenuserrow:
				...
				if row:
					...
					if not sessionrowlist:
						return errorresponse("ERROR_session_not_found")
					sessionrow=sessionrowlist[0]
					access=sessionaccess(sessionrow,tokenuserrow[0])
					playerrow=query(SETTING["dbname"],"""SELECT*FROM "sessionplayer" WHERE "sessionid"=%s AND "userid"=%s AND "deletetime" IS NULL""",[sessionrow["id"],tokenuserrow[0]["id"]],SETTING["dbsetting"])
					if access["isown"] or access["isadmin"] or access["isstaff"] or access["isplayer"] or playerrow:
						seatingdata=[]
						handdata=[]
						ev_actual=[]
