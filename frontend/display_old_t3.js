...
function averagestacktotal() {
	// 取兩種估法的較大值:
	// 1) 計分牌守恆:起始碼 × 總入場人次。單日賽在沒有牌局資料時最準——被淘汰選手的
	//    計分牌已轉給存活者不會消失,只加總存活者當前碼(averageStackTotal)會低估。
	// 2) 後端 averageStackTotal:在場選手當前計分牌總和。多日賽 Day2 起必須用它——
	//    晉級者各自帶入不同碼量(advancechip),守恆公式的「起始碼×人次」會嚴重低估
	//    (實際發生過: 3000×14=42000 vs 真實帶入 351000, 平均被顯示成 3000)。
	// 單日賽守恆值較大、多日賽後端值較大,取 max 兩種賽制都對,不需依 showMultidayRemaining 分支。
	let conserved=numericstate(state["startingChips"]) * numericstate(state["totalEntries"])
	let reported=numericstate(state["averageStackTotal"])
	return Math.max(conserved, reported)
}
