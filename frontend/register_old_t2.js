...
function tablemanagelabel(table){
	return String(table["no"]||table["name"]||table["token"]||table["id"])
}

function tableoccupiedcount(tableid){
	let count=0
	for(let i=0;i<registrationdatalist.length;i=i+1){
		let row=registrationdatalist[i]
		let seated=row["tableid"]&&row["seatno"]&&row["status"]!="cancelled"&&row["timerstatus"]!="eliminated"
		if(seated&&String(row["tableid"])==String(tableid)){
			count=count+1
		}
	}
	return count
...
function rendertablemanage(){
	let box=domgetid("tablemanagelist")
	if(!box){
		return
	}
	innertext("#tablemanagetitle",tmt("sectiontitle","牌桌管理"),false)
	innertext("#tablemanagedesc",tmt("sectiondesc","可對每桌直接併桌或關閉；已關閉的桌不會再有新選手進入。"),false)
	let html=""
	for(let i=0;i<tables.length;i=i+1){
		let table=tables[i]
		let closed=table["closedtime"]?true:false
		let occupied=tableoccupiedcount(table["id"])
		let badge=""
		if(closed){
			badge=`<span class="rounded-full border border-rose-500/40 bg-rose-500/10 px-2 py-0.5 text-xs font-bold text-rose-300">${tmt("closedbadge","已關閉")}</span>`
		}
		html=html+`
			<div class="rounded border border-zinc-700 bg-zinc-800 p-3">
				<div class="mb-2 flex items-center justify-between gap-2">
					<div class="min-w-0 truncate text-sm font-bold text-white">${safehtml(tablemanagelabel(table))}</div>
					${badge}
				</div>
				<div class="mb-2 text-xs text-zinc-400">${tmt("playercount","{occupied}/{maxseat} 人").replace("{occupied}",occupied).replace("{maxseat}",maxseat)}</div>
				<div class="flex flex-wrap gap-2">
					<input type="button" class="tablemergebtn bg-amber-600 hover:bg-amber-700 px-3 py-1 rounded text-xs font-semibold" data-id="${safehtml(table["id"])}" value="${tmt("mergebutton","併桌")}">
					<input type="button" class="tableclosebtn ${closed?"bg-emerald-600 hover:bg-emerald-700":"bg-zinc-700 hover:bg-zinc-600"} px-3 py-1 rounded text-xs font-semibold" data-id="${safehtml(table["id"])}" data-closed="${closed?"1":""}" value="${closed?tmt("openbutton","重新開啟"):tmt("closebutton","關閉牌桌")}">
				</div>
			</div>
		`
	}
	if(!html){
		html=`<div class="text-sm text-zinc-500">${tmt("notables","此場次尚無牌桌")}</div>`
	}
	box.innerHTML=html
	onclick(".tablemergebtn",function(element,event){
		opentablemergemodal(dataset(element,"id"))
	})
	onclick(".tableclosebtn",function(element,event){
		let tableid=dataset(element,"id")
		let closed=dataset(element,"closed")?false:true
		let table=findtable(tableid)
		let label=table?tablemanagelabel(table):""
		if(closed){
			ptconfirm(tmt("closeconfirm","確定要關閉「{table}」嗎？關閉後不會再有新選手進入此桌。").replace("{table}",safehtml(label)),function(okayed){
				if(!okayed){
					return
				}
				calltableclosed(tableid,true,element)
			})
		}else{
			calltableclosed(tableid,false,element)
		}
	})
}

function calltableclosed(tableid,closed,element){
	if(element){
	...
	ajax("POST",AJAXURL+"closetable/"+tableid,function(event,data){
		...
		if(data["success"]){
			pttoast(closed?tmt("closesuccess","已關閉牌桌"):tmt("opensuccess","已重新開啟牌桌"),"success")
			loadregistrations()
		}else{
			pttoast(pterror(data["data"]||tmt("togglefail","操作失敗")),"error")
		}
	},str({
		"closed": closed
	}),[
		...
		loadingtarget: "#tablemanagelist"
	})
}

function opentablemergemodal(sourceid){
	let source=findtable(sourceid)
	if(!source){
		return
	}
	let sourcelabel=tablemanagelabel(source)
	let sourcecount=tableoccupiedcount(sourceid)
	...
	for(let i=0;i<tables.length;i=i+1){
		let table=tables[i]
		if(String(table["id"])==String(sourceid)||table["closedtime"]){
			continue
		}
		let empty=maxseat-tableoccupiedcount(table["id"])
		let enough=sourcecount<=empty
		let optiontext=tablemanagelabel(table)+"　"+tmt("emptycount","{n} 空位").replace("{n}",empty)
		if(!enough){
			optiontext=optiontext+"　"+tmt("mergeinsufficient","空位不足")
		}
	...
	if(!optionhtml){
		optionhtml=`<div class="text-sm text-zinc-500">${tmt("mergenotarget","沒有可併入的開放牌桌")}</div>`
	}
	let cover=doccreate("div")
	...
	cover.innerHTML=`
		<div class="bg-zinc-900 border border-zinc-700 rounded-lg max-w-md w-full p-5 shadow-xl">
			<div class="flex items-center justify-between mb-4">
				<div class="text-lg font-semibold text-white">${tmt("mergetitle","併桌：選擇目標牌桌")}</div>
				<input type="button" class="closetablemerge text-zinc-400 hover:text-white" value="×">
			</div>
			<div class="text-sm text-zinc-400 mb-4">${tmt("mergedesc","把「{table}」的所有選手併到選定的目標桌，併桌後本桌會自動關閉。").replace("{table}",safehtml(sourcelabel))}</div>
			<div class="grid grid-cols-1 gap-2 max-h-[50vh] overflow-y-auto">${optionhtml}</div>
			<div class="flex justify-end gap-2 mt-5">
				<input type="button" class="closetablemerge bg-zinc-700 hover:bg-zinc-600 px-4 py-2 rounded" value="${tmt("cancel","取消")}">
			</div>
		</div>
	`
	ptlockpagescroll()
	...
	onclick(".tablemergetargetbtn",function(element,event){
		...
		let target=findtable(targetid)
		let targetlabel=target?tablemanagelabel(target):""
		ptconfirm(tmt("mergeconfirm","確定要把「{source}」的所有選手併到「{target}」嗎？併桌後「{source}」會自動關閉。").replace(/\{source\}/g,safehtml(sourcelabel)).replace("{target}",safehtml(targetlabel)),function(okayed){
			if(!okayed){
				return
			}
			mergetableto(sourceid,targetid)
		})
	})
}

function mergetableto(sourceid,targetid){
		...
		if(data["success"]){
			...
			ajax("POST",AJAXURL+"closetable/"+sourceid,function(event,closedata){
				if(closedata["success"]){
					opentablemergeresult(movedata,true)
				}else{
					pttoast(tmt("mergeclosefail","併桌完成，但自動關閉來源桌失敗，請手動關閉"),"warning")
					opentablemergeresult(movedata,false)
				}
				loadregistrations()
			},str({
				"closed": true
			...
			pttoast(pterror(data["data"]||tmt("mergefail","併桌失敗")),"error")
		}
	},str({
...
function opentablemergeresult(movedata,closeded){
	...
	if(!rowhtml){
		rowhtml=`<div class="py-3 text-sm text-zinc-400">${tmt("resultempty","此次併桌沒有需要移動的選手。")}</div>`
	}
	let closedhtml=""
	if(closeded){
		closedhtml=`<div class="mt-3 text-xs text-amber-300">${tmt("resultclosed","「{source}」已自動關閉，不會再有新選手進入。").replace("{source}",safehtml(sourcelabel))}</div>`
	}
	let cover=doccreate("div")
	...
	cover.innerHTML=`
		<div class="bg-zinc-900 border border-zinc-700 rounded-lg max-w-md w-full p-5 shadow-xl">
			<div class="text-lg font-semibold text-white mb-4">${tmt("resulttitle","「{source}」併入「{target}」").replace("{source}",safehtml(sourcelabel)).replace("{target}",safehtml(targetlabel))}</div>
			<div class="max-h-[50vh] overflow-y-auto">${rowhtml}</div>
			${closedhtml}
			<div class="flex justify-end gap-2 mt-5">
				<input type="button" class="closemergeresult bg-emerald-600 hover:bg-emerald-700 px-4 py-2 rounded font-semibold" value="${tmt("resultclose","關閉")}">
			</div>
		</div>
	`
	ptlockpagescroll()
		...
		if(toolbar){
			...
			toolbar.innerHTML=`
				...
				<div class="mt-4 border-t border-zinc-800 pt-4">
					<div class="mb-2 text-xs font-bold uppercase tracking-[0.16em] text-emerald-400" id="tablemanagetitle"></div>
					<div class="mb-3 text-sm leading-6 text-zinc-400" id="tablemanagedesc"></div>
					<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2" id="tablemanagelist"></div>
				</div>
				<div class="mt-4 border-t border-zinc-800 pt-4">
