...
function openboardmergeresult(movedata,closeded){
	...
	cover.innerHTML=`
		<div class="bg-zinc-900 border border-zinc-700 rounded-lg max-w-md w-full p-5 shadow-xl">
			<div class="text-lg font-semibold text-white mb-4">${boardmanagetext("resulttitle","「{source}」併入「{target}」").replace("{source}",boardescape(sourcelabel)).replace("{target}",boardescape(targetlabel))}</div>
